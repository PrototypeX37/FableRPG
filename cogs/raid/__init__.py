...
class Raid(commands.Cog):
    ...
    async def spawn(self, ctx, hp: IntGreaterThan(0), rarity: str = "magic", raid_hp: int = 250):
        ...
        if self.bot.config.bot.is_beta:
            ...
            await ctx.send(content, allowed_mentions=discord.AllowedMentions(roles=True))
            ...
            await ctx.send("**Ragnarok will be vulnerable in 10 seconds**")
        ...
        await ctx.send("**Ragnarok is vulnerable! Fetching participant data... Hang on!**")
        ...
        if len(self.raid) == 0:
            ...
            if not self.raid:
                ...
                msg = await ctx.send(
                    ...
                    f" **${highest_bid[1]}**!\nGiving {rarity.capitalize()} Crate..."
                )
                ...
                await ctx.send(f"**Gave ${cash:,.0f} of Ragnarok's ${cash_pool:,.0f} drop to all survivors!**")
                ...
                if self.boss["hp"] > 1:
                    m = await ctx.send("The raid did not manage to kill Ragnarok within 45 Minutes... He disappeared!")
                    await m.add_reaction("\U0001F1EB")
                    summary_text = (
            ...
            if self.bot.config.bot.is_beta:
                ...
                try:
                    ...
                    await summary_channel.send(
                        ...
                        # Uncomment and modify the below line if you need mentions:
                        # f" {ctx.author.mention}"
                    )
                else:
                    await summary_channel.send("Message has been published!", delete_after=10)
            ...
            **Only followers of the Sepulchure may join.**
            ...
            if progress >= 100:
                ...
                embed = Embed(
                    ...
                    description="The night resonates with dark energy as Sepulchure's power reaches its zenith. The ritual has been successfully consummated! As a token of Sepulchure's dark favor, one chosen acolyte will be granted a legendary crate. The rest, fear not, for riches of the shadow realm shall be your reward.",
                ...
                await ctx.send(embed=embed)
                await ctx.send(
                    f"🦇 Dark tidings, <@{random_user}>! You have been selected as the chosen acolyte, and you shall receive a legendary crate in the Ritual of Sepulchure. 💀")
                await self.bot.pool.execute(
                    'UPDATE profile SET crates_legendary=crates_legendary+$1 WHERE "user"=$2;',
                    1,
                    random_user  # No need to wrap in a list
                )
...
**Only followers of Jesus may join.**""",
    ...
    async def raidstats(self, ctx):
        ...
        hp = ctx.character_data["health"] + 250
