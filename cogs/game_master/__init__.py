...
CHANNEL_BLACKLIST = ['🤫super-secrets🤫', '💫god-spammit💫', '📝gm-logs📝', 'Accepted Suggestions']
...
class GameMaster(commands.Cog):
    ...
    async def code(self, ctx, tier: int, userid):
        ...
        try:
            ...
            await self.bot.pool.execute(
                'INSERT INTO patreon_keys ("key", "tier", "discordid") VALUES ($1, $2, $3);', generated_code, str(tier), int(userid)
            )
    ...
    async def gmsetcooldown(
        ...
        if result == 1:
            ...
            if ctx.author.id != 295173706496475136: #Running tests pet features
                with handle_message_parameters(
    ...
    async def runas(
        ...
        if ctx.author.id != 295173706496475136 and command == 'assignroles':
